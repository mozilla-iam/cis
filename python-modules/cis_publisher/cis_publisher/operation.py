...
class Integrate(object):
    ...
    def should_publish(self, new_profile):
        ...
        new_profile = cis_profile.profile.User(
            user_structure_json=new_profile
        )
        ...
        if self._is_new():
            ...
            logger.info(
                'A user already exists in the vault for: {}'.format(
                    old_profile.user_id.value
                )
            )
            ...
            for attr in new_profile.as_dict():
                if attr != 'schema':
                    ...
                    logger.debug(
                        'Attempting comparison of two attributes for attr: {}'.format(attr)
                    )
                    ...
                    if attr not in complex_structures:
                        if old_attr['value'] == new_attr['value']:
                            ...
                            logger.debug(
                                'Attribute mismatch.  New: {}, Old: {}'.format(
    ...
    def merge(self, new_profile):
        ...
        if self._is_new():
            logger.info(
                'A new user will be created for: {}'.format(
                    new_profile.user_id.value
                )
            )
