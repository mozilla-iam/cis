...
class TestOperation(object):
    ...
    def test_verify_with_http_retrieval(self, mock_get):
        ...
        assert test_valid_payload is not None
        sig = s.jws()
        mock_resp = mock_resp = self._mock_response(json_data=well_known.MozillIAM().data())
    ...
    @mock.patch('requests.get')
    def test_strict_verify_with_http_retrieval_it_should_fail(self, mock_get):
        from cis_crypto import operation
        os.environ['CIS_SECRET_MANAGER_FILE_PATH'] = 'tests/fixture'
        os.environ['CIS_SECRET_MANAGER'] = 'file'
        os.environ['CIS_SIGNING_KEY_NAME'] = 'fake-access-file-key.priv.pem'
        ...
        publishers = fake_well_known_json.get('publishers_supported')
        ...
        for publisher in publishers:
            with pytest.raises(JWSError):
                o.jws()
                o.jws(publisher_authority=publisher)

    @mock.patch('requests.get')
    def test_strict_verify_with_http_retrieval_it_should_succeed(self, mock_get):
        from cis_crypto import operation
        os.environ['CIS_SECRET_MANAGER_FILE_PATH'] = 'tests/fixture'
        os.environ['CIS_SECRET_MANAGER'] = 'file'
        os.environ['CIS_SIGNING_KEY_NAME'] = 'fake-publisher-key_0.priv.pem'
        os.environ['CIS_WELL_KNOWN_MODE'] = 'http'
        os.environ['CIS_WELL_KNOWN_URL'] = 'http://127.0.0.1:5000/.well-known/mozilla-iam'
        os.environ['CIS_STRICT_VERIFICATION'] = 'True'
        ...
        # Add our fake guaranteed to pass publisher to the list
        guaranteed_to_pass_publisher = {'winning_publisher': {'jwks_keys': [fixture_object.material]}}
        fake_well_known_json['publishers_supported'] = guaranteed_to_pass_publisher
        mock_well_known = self._mock_response(json_data=fake_well_known_json)
        mock_get.return_value = mock_well_known
        o = operation.StrictVerify()
        o.load(sig)
        res = o.jws(publisher_authority='winning_publisher')
        assert res is not None
